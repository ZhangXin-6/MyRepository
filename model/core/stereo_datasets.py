...
class StereoDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        cls = frame_utils.read_gen(self.cls_list[index])
        ...
        if self.augmentor is not None:
            if self.sparse:
                img1, img2, flow, valid, cls = self.augmentor(img1, img2, flow, valid, cls)
            else:
                img1, img2, flow, cls = self.augmentor(img1, img2, flow, cls)
        ...
        cls = torch.from_numpy(cls).permute(2, 0, 1).float()
        ...
        flow = flow[:1]
        return self.image_list[index] + [self.disparity_list[index]] + [self.cls_list[index]], img1, img2, flow, valid.float(), cls
    ...
    def __mul__(self, v):
        ...
        copy_of_self.cls_list = v * copy_of_self.cls_list
...
class ETH3D(StereoDataset):
    def __init__(self, aug_params=None, root='./ETH3D', split='training'):
        super(ETH3D, self).__init__(aug_params, sparse=True, reader=frame_utils.readDispETH3D)

        image1_list = sorted(glob(osp.join(root, f'two_view_{split}/*/im0.png')))
        image2_list = sorted(glob(osp.join(root, f'two_view_{split}/*/im1.png')))
        disp_list = sorted(glob(osp.join(root, 'two_view_training_gt/*/disp0GT.pfm'))) \
            if split == 'training' else [osp.join(root, 'two_view_training_gt/playground_1l/disp0GT.pfm')] * len(image1_list)
...
class KITTI(StereoDataset):
    def __init__(self, aug_params=None, root='datasets/KITTI', image_set='training'):
        ...
        disp_list = sorted(glob(os.path.join(root, 'training', 'disp_occ_0/*_10.png'))) \
...
class US3D(StereoDataset):
    def __init__(self, aug_params=None, root='./US3D', image_set='train'):
        super(US3D, self).__init__(aug_params, sparse=True, reader=frame_utils.readDispUS3D)
        assert os.path.exists(root)

        image1_list = sorted(glob(os.path.join(root, image_set, 'left/*.tif')))
        image2_list = sorted(glob(os.path.join(root, image_set, 'right/*.tif')))
        disp_list = sorted(glob(os.path.join(root, image_set, 'disp/*.tif')))
        cls_list = sorted(glob(os.path.join(root, image_set, 'cls/*.tif')))

        for idx, (img1, img2, disp, cls) in enumerate(zip(image1_list, image2_list, disp_list, cls_list)):
            self.image_list += [[img1, img2]]
            self.disparity_list += [disp]
            self.cls_list += [cls]


class GF7(StereoDataset):
    def __init__(self, aug_params=None, root='./GF7', image_set='train'):
        super(GF7, self).__init__(aug_params, sparse=True, reader=frame_utils.readDispGF7)
        assert os.path.exists(root)

        image1_list = sorted(glob(os.path.join(root, image_set, 'left/*.tiff')))
        image2_list = sorted(glob(os.path.join(root, image_set, 'right/*.tiff')))
        disp_list = sorted(glob(os.path.join(root, image_set, 'disp/*.tiff')))
...
def fetch_dataloader(args):
    ...
    for dataset_name in args.train_datasets:
        ...
        elif dataset_name == 'GF7':
            new_dataset = GF7(aug_params)
            logging.info(f"Adding {len(new_dataset)} samples from GF7")
