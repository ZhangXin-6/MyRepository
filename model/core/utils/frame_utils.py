...
def read_gen(file_name, pil=False):
    ...
    if ext == '.png' or ext == '.jpeg' or ext == '.ppm' or ext == '.jpg' or ext == '.tiff' or ext == '.tif':
...
def readDispUS3D(filename):
    disp = cv2.imread(filename, cv2.IMREAD_ANYDEPTH)
    valid = (disp > -128) & (disp < 128)
    return disp, valid


def readDispGF7(filename):
    disp = cv2.imread(filename, cv2.IMREAD_ANYDEPTH)
    valid = (disp > -256) & (disp < 256)
    return disp, valid


def readDispETH3D(filename):
    disp = cv2.imread(filename, cv2.IMREAD_ANYDEPTH)
    valid = disp > 0.0
    return disp, valid
