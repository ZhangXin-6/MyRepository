...
class FlowAugmentor:
    ...
    def color_transform(self, img1, img2):
        ...
        if np.random.rand() < self.asymmetric_color_aug_prob:
            img1 = np.array(self.photo_aug(Image.fromarray(img1)), dtype=np.uint8)  #np.uint8
            img2 = np.array(self.photo_aug(Image.fromarray(img2)), dtype=np.uint8)  #np.uint8
    ...
    def spatial_transform(self, img1, img2, flow, cls):
        ...
        if np.random.rand() < self.spatial_aug_prob:
            ...
            cls = cv2.resize(cls, None, fx=scale_x, fy=scale_y, interpolation=cv2.INTER_LINEAR)
        ...
        if self.yjitter:
            ...
            cls = cls[y0:y0 + self.crop_size[0], x0:x0 + self.crop_size[1]]
        ...
        return img1, img2, flow, cls

    def __call__(self, img1, img2, flow, cls):
        img1, img2 = self.color_transform(img1, img2)
        img1, img2 = self.eraser_transform(img1, img2)
        img1, img2, flow, cls = self.spatial_transform(img1, img2, flow, cls)
...
class SparseFlowAugmentor:
    ...
    def spatial_transform(self, img1, img2, flow, valid, cls):
        # randomly sample scale
        ...
        return img1, img2, flow, valid, cls

    def __call__(self, img1, img2, flow, valid, cls):
        img1, img2 = self.color_transform(img1, img2)
        img1, img2 = self.eraser_transform(img1, img2)
        img1, img2, flow, valid, cls = self.spatial_transform(img1, img2, flow, valid, cls)
